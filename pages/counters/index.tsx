...
const clinicSeta = (code:string) => {
 ...
 const submit  = (e:React.FormEvent) => {
    e.preventDefault()
    axios.post("http://localhost:5005/counters/create_counter",{service:fields.service,namba: fields.namba, clinic: fields.clinic,code: fields.code}).then((data:any)=> {
        setAdd(false)
        router.reload()
    }).catch((error)=> {
 ...
 const deleteService  = (id:string) => {
    axios.put(`http://localhost:5005/counters/delete_counter/${id}`).then(()=> {
        router.reload()
    }).catch((error)=> {
        if (error.response && error.response.status === 400) {
            console.log(`there is an error ${error.message}`)
 ...
 const getServices  = () => {
    ...
    axios.get("http://localhost:5005/counters/get_counters",{params: {page,pagesize}}).then((data)=> {
