...
const {data} = useFetchData("http://192.168.30.246:5005/services/get_all_services")
...
useEffect(()=> {
 ...
 const submit  = (e:React.FormEvent) => {
    e.preventDefault()
    axios.post("http://192.168.30.246:5005/doctors/create_doctor",{name:fields.name,phone:fields.phone,service:fields.service,room:fields.counter,clinic: fields.clinic,clinic_code: fields.clinic_code}).then((data:any)=> {
        setAdd(false)
        router.reload()
    }).catch((error)=> {
 ...
 const deleteService  = (id:string) => {
    axios.put(`http://192.168.30.246:5005/doctors/delete_doctor/${id}`).then(()=> {
        router.reload()
    }).catch((error)=> {
        if (error.response && error.response.status === 400) {
            setMessage({...onmessage,title:error.response.data.error,category: "error"})
 ...
 const editService  = (e:React.FormEvent) => {
    e.preventDefault()
    axios.put(`http://192.168.30.246:5005/doctors/edit_doctor/${id}`,{fields}).then((data:any)=> {
        setEdit(false)
        router.reload()
    }).catch((error)=> {
        if (error.response && error.response.status === 400) {
 ...
 const getAttendants  = () => {
    ...
    axios.get("http://192.168.30.246:5005/doctors/get_doctors",{params: {page,pagesize}}).then((data)=> {
