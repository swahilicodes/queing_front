...
const [finiLoading, setFiniLoading] = useState(false)
  ...
  const getPatients = () => {
    ...
    axios.get('http://localhost:5000/patients/get_account_patients',{params: {status,mr_no,stage:"accounts"}}).then((data)=> {
  ...
  const finishPatient = (id:string) => {
    setFiniLoading(true)
    axios.put(`http://localhost:5000/patients/finish_accounts_patient/${id}`,{service:pay}).then((data:any)=> {
      socket.emit("data",{data:data.data,route:"tickets"})
      setInterval(()=> {
        setFiniLoading(false)
        setFinish(false)
        router.reload()
      },3000)
    }).catch((error)=> {
            ...
            : <div className={styles.wrap}>
                {
                    ...
                    : <div className={styles.list_queue_list}>
                        ...
                        <div className={cx(styles.saving,full && styles.full)}>
                          ...
                          <div className={styles.finish} onClick={()=> prepareFinish(clients[0].id)}>Finish</div>
          ...
          clients.length > 0 && (<div className={styles.bottom_desc}>
            <div className={styles.top}>
              <div className={styles.item}>
                <p>On Queue: <span> <Medicine_Category_Length stage="accounts" status='waiting'/> </span> </p>
              </div>
              <div className={styles.item}>
                <p>Attended: <span><Medicine_Category_Length stage="accounts" status='done'/></span> </p>
              </div>
              <div className={styles.item}>
                <p>Pending: <span><Medicine_Category_Length stage="accounts" status='pending'/></span> </p>
              </div>
              <div className={styles.item}>
                <p>Cancelled: <span><Medicine_Category_Length stage="accounts" status='cancelled'/></span> </p>
              </div>
