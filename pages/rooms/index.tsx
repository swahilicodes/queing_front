...
const {data,loading,error} = useFetchData("http://192.168.30.246:5000/clinic/get_clinics")
...
const clinicSeta = (code:string) => {
 ...
 const submit  = (e:React.FormEvent) => {
    e.preventDefault()
    axios.post("http://192.168.30.246:5000/rooms/create_room",{namba: fields.namba, clinic: fields.clinic,clinic_code: fields.code}).then((data:any)=> {
        setAdd(false)
        router.reload()
    }).catch((error:any)=> {
 ...
 const deleteService  = (id:any) => {
    axios.put(`http://192.168.30.246:5000/counters/delete_counter/${id}`).then((data:any)=> {
        router.reload()
    }).catch((error:any)=> {
        if (error.response && error.response.status === 400) {
            setMessage({...onmessage,title:error.response.data.error,category: "error"})
 ...
 const getServices  = () => {
    ...
    axios.get("http://192.168.30.246:5000/rooms/get_rooms",{params: {page,pagesize, clinic_code: fields.code}}).then((data:any)=> {
