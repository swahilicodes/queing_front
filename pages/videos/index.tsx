...
function Videos() {
    ...
    const uploadFinal = (url:string) => {
        axios.post("http://192.168.30.246:5005/uploads/upload-video",{name: fields.name,description: fields.description,url: url}).then((data)=> {
    ...
    const deleteVideo = (id:number,url:string) => {
        ...
        axios.post("http://192.168.30.246:5005/uploads/delete_video",{id:id, url: url}).then((data)=> {
        location.reload()
        }).catch((error)=> {
            if (error.response && error.response.status === 400) {
                console.log(`there is an error ${error.message}`)
    ...
    const uploadVideo = async (e: React.FormEvent) => {
        ...
        try {
            axios.post("http://192.168.30.246:5005/uploads/upload", formData, {
                headers: {
