...
export default function Admins() {
    ...
    const { data: clinics } = useFetchData("http://192.168.30.245:5000/clinic/get_clinics")
    ...
    const handleClinic = (code: string) => {
        const clinic = clinics.find((item: { clinicicode: string }) => item.clinicicode === code)
        setFields({ ...fields, clinic: clinic.cliniciname, clinic_code: code })
    }

    const createClinic = (deviceId: string) => {
        axios.post(`http://192.168.30.245:5000/attendant_clinics/create_attendant_clinic`, { clinic_code: fields.clinic_code, clinic: fields.clinic, attendant_id: deviceId }).then((data) => {
            //setPat(data.data)
    ...
    const deleteClinic = (clinic_code: string, device_id: string) => {
        axios.get(`http://192.168.30.245:5000/attendant_clinics/delete_clinic`, { params: { clinic_code: clinic_code, attendant_id: device_id } }).then((data) => {
    ...
    const getDocClinics = (device_id: string) => {
        axios.get(`http://192.168.30.245:5000/attendant_clinics/get_clinics`, { params: { attendant_id: device_id } }).then((data) => {
    ...
    const editService = (e: React.FormEvent) => {
        e.preventDefault()
        axios.get(`http://192.168.30.245:5000/network/edit_device`, { params: { page: fields.page, id: id } }).then(() => {
            setEdit(false)
            router.reload()
        }).catch((error) => {
            if (error.response && error.response.status === 400) {
    ...
    const deleteService = () => {
        axios.get(`http://192.168.30.245:5000/network/delete_device`, { params: { id: id } }).then(() => {
            setDelete(false)
            router.reload()
        }).catch((error) => {
            if (error.response && error.response.status === 400) {
    ...
    const getAttendants = () => {
        ...
        axios.get("http://192.168.30.245:5000/network/get_devices", { params: { page, pagesize } }).then((data) => {
                    ...
                    </div>
                    {
                        isAddittion
                            ? <div className={styles.addittion}>
                                ...
                                <button onClick={() => createClinic(fields.device_id)}>submit</button>
                            </div>
                            : <div className={styles.display_clinics}>
                                {
                                    attendantClinics.map((item: any, index) => (
                                        <div className={styles.display_item} key={index}>
                                            ...
                                            <div className={styles.delete} onClick={() => deleteClinic(item.clinic_code, fields.device_id)}>
                                                    ...
                                                    services.map((data: any, index: number) => (
                                                        <tr key={index} className={cx(index % 2 === 0 && styles.active)}>
                                                            ...
                                                            <td>
                                                                ...
                                                                <div className={styles.edit} onClick={() => handleAdd(data.macAddress)}><IoIosAdd className={styles.icon} /></div>
