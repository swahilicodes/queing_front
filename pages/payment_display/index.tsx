...
//  }
    ...
    const getTickets = () => {
      axios.get('http://localhost:5000/patients/get_pats',{params: {stage:"payment"}}).then((data)=> {
            ...
            ? <div className={styles.queue_div}>
              {
                tickets.map((item:any,index:number)=> (
                  <div className={cx(styles.div,index===0 && styles.serving,index===1 && styles.next)}>
                    ...
                    <FaArrowTrendUp className={styles.con} size={40}/>
                    <span>COUNTER <span>{(item.counter===undefined || item.counter.namba === undefined)?"000":item.counter.namba}</span> </span>
                    </div>
                  </div>
                ))
              }
