...
const {data:clinics,loading:srsLoading, error: srsError} = useFetchData("https://qms-back.mloganzila.or.tz/clinic/get_clinics")
...
useEffect(()=> {
 ...
 const submit  = (e:React.FormEvent) => {
    e.preventDefault()
    axios.post("https://qms-back.mloganzila.or.tz/doktas/create_dokta",{name:fields.name,phone:fields.phone,room:fields.room,clinic: fields.clinic,clinic_code: fields.clinic_code}).then(()=> {
        setAdd(false)
        router.reload()
    }).catch((error)=> {
 ...
 const deleteService  = (id:string) => {
    axios.put(`https://qms-back.mloganzila.or.tz/doktas/delete_dokta/${id}`).then(()=> {
        router.reload()
    }).catch((error)=> {
        if (error.response && error.response.status === 400) {
            console.log(`there is an error ${error.message}`)
 ...
 const getRooms  = () => {
    axios.get(`https://qms-back.mloganzila.or.tz/rooms/get_rooms`,{params: {clinic_code: fields.clinic_code}}).then((data)=> {
...
//     e.preventDefault()
//     axios.put(`https://qms-back.mloganzila.or.tz/doctors/edit_doctor/${id}`,{fields}).then(()=> {
//         setEdit(false)
//         router.reload()
//     }).catch((error:any)=> {
//         if (error.response && error.response.status === 400) {
...
//  }
 ...
 const getAttendants  = () => {
    ...
    axios.get("https://qms-back.mloganzila.or.tz/doktas/get_doktas",{params: {page,pagesize,clinic_code: fields.clinic_code}}).then((data)=> {
