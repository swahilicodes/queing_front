...
useEffect(()=> {
 ...
 const submit  = (e:React.FormEvent) => {
    e.preventDefault()
    axios.post("http://localhost:5005/services/create_service",{name}).then(()=> {
        setAdd(false)
        router.reload()
    }).catch((error)=> {
 ...
 const deleteService  = (id:string) => {
    axios.put(`http://localhost:5005/services/delete_service/${id}`).then(()=> {
        router.reload()
    }).catch((error)=> {
        if (error.response && error.response.status === 400) {
            setMessage({...onmessage,title:error.response.data.error,category: "error"})
 ...
 const editService  = (e:React.FormEvent) => {
    e.preventDefault()
    axios.put(`http://localhost:5005/services/edit_service/${id}`,{name}).then(()=> {
        setEdit(false)
        router.reload()
    }).catch((error)=> {
        if (error.response && error.response.status === 400) {
 ...
 const getServices  = () => {
    ...
    axios.get("http://localhost:5005/services/get_services",{params: {page,pagesize}}).then((data)=> {
