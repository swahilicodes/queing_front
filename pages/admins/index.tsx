...
useEffect(()=> {
 ...
 const submit  = (e:React.FormEvent) => {
    e.preventDefault()
    axios.post("http://192.168.30.246:5000/admins/create_admin",{name,phone,service:"administration",counter:"101",role:"admin"}).then((data:any)=> {
        setAdd(false)
        router.reload()
    }).catch((error:any)=> {
 ...
 const deleteService  = (id:string) => {
    axios.put(`http://192.168.30.246:5000/admins/delete_admin/${id}`).then(()=> {
        router.reload()
    }).catch((error)=> {
        if (error.response && error.response.status === 400) {
            setMessage({...onmessage,title:error.response.data.error,category: "error"})
 ...
 const editService  = (e:React.FormEvent) => {
    e.preventDefault()
    axios.put(`http://192.168.30.246:5000/services/edit_service/${id}`,{name}).then((data:any)=> {
        setEdit(false)
        router.reload()
    }).catch((error)=> {
        if (error.response && error.response.status === 400) {
 ...
 const getAttendants  = () => {
    ...
    axios.get("http://192.168.30.246:5000/admins/get_admins",{params: {page,pagesize}}).then((data)=> {
