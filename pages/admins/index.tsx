...
useEffect(()=> {
 ...
 const submit  = (e:React.FormEvent) => {
    e.preventDefault()
    axios.post("https://qms-back.mloganzila.or.tz/admins/create_admin",{name,phone,service:"administration",counter:"101",role:"admin"}).then((data:any)=> {
        setAdd(false)
        router.reload()
    }).catch((error:any)=> {
 ...
 const deleteService  = (id:string) => {
    axios.put(`https://qms-back.mloganzila.or.tz/admins/delete_admin/${id}`).then(()=> {
        router.reload()
    }).catch((error)=> {
        if (error.response && error.response.status === 400) {
            console.log(`there is an error ${error.message}`)
 ...
 const editService  = (e:React.FormEvent) => {
    e.preventDefault()
    axios.put(`https://qms-back.mloganzila.or.tz/services/edit_service/${id}`,{name}).then((data:any)=> {
        setEdit(false)
        router.reload()
    }).catch((error)=> {
        if (error.response && error.response.status === 400) {
 ...
 const getAttendants  = () => {
    ...
    axios.get("https://qms-back.mloganzila.or.tz/admins/get_admins",{params: {page,pagesize}}).then((data)=> {
