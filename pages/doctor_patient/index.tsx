...
export default function DoctorPatient() {
    ...
    const getDoktas = () => {
        axios.get("http://192.168.30.246:5005/doktas/get_all_doktas")
            .then((data) => {
                setDoktas(data.data)
            })
            .catch((error) => {
    ...
    const editDoctor = (e: React.FormEvent) => {
        ...
        axios.post("http://192.168.30.246:5005/doktas/edit_doctor_one", {
            name: fields.name, 
    ...
    const editTicket = (id: number, status: string) => {
        ...
        axios.put(`http://192.168.30.246:5005/tickets/edit_ticket/${id}`, { status })
            .then(() => {
                setTimeout(() => {
    ...
    const getDocPat = () => {
        setFetchLoading(true)
        axios.get(`http://192.168.30.246:5005/doktas/get_doc_patients`, {
            params: { status,page },
            headers: {
                Authorization: `Bearer ${localStorage.getItem('token')}`
            }
    ...
    const createClinic = () => {
        axios.post(`http://192.168.30.246:5005/attendant_clinics/create_attendant_clinic`, { 
    ...
    const deleteClinic = (clinic_code: string) => {
        axios.get(`http://192.168.30.246:5005/attendant_clinics/delete_clinic`, { 
    ...
    const getDocClinics = () => {
        axios.get(`http://192.168.30.246:5005/attendant_clinics/get_clinics`, { 
    ...
    const finishToken = (patient_id: string) => {
        setFinLoading(true)
        axios.post("http://192.168.30.246:5005/doktas/finish_patient", { 
                            ...
                            <div className={cx(styles.spika, loading && styles.active)} onClick={() => setSpeaker(!isSpeaker)}>
                                <div 
                                    ...
                                    onClick={() => createItem(
                                        ...
                                        "clinic", 
                                        "m02", 
                                        "http://192.168.30.246:5005/speaker/create_speaker", 
