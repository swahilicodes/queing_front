...
useEffect(()=> {
 ...
 const submit  = (e:React.FormEvent) => {
    e.preventDefault()
    axios.post("http://192.168.30.246:5005/adverts/create_advert",{name,description}).then((data:any)=> {
        setAdd(false)
        router.reload()
    }).catch((error)=> {
 ...
 const deleteService  = (id:string) => {
    axios.put(`http://192.168.30.246:5005/adverts/delete_advert/${id}`).then(()=> {
        router.reload()
    }).catch((error)=> {
        if (error.response && error.response.status === 400) {
            setMessage({...onmessage,title:error.response.data.error,category: "error"})
 ...
 const editAdvert  = (name:string,description:string) => {
    axios.put(`http://192.168.30.246:5005/adverts/edit_advert/${id}`,{id: id,name,description}).then((data:any)=> {
        setEdit(false)
        //console.log(data)
        router.reload()
    }).catch((error)=> {
 ...
 const getAttendants  = () => {
    ...
    axios.get("http://192.168.30.246:5005/adverts/get_adverts",{params: {page,pagesize}}).then((data)=> {
