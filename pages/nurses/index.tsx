...
useEffect(()=> {
 ...
 const submit  = (e:React.FormEvent) => {
    e.preventDefault()
    axios.post("http://localhost:5000/nurse/create_nurse",{name,phone,service,room:counter}).then((data:any)=> {
        setAdd(false)
        router.reload()
    }).catch((error:any)=> {
 ...
 const deleteService  = (id:any) => {
    axios.put(`http://localhost:5000/nurse/delete_nurse/${id}`).then((data:any)=> {
        router.reload()
    }).catch((error:any)=> {
        if (error.response && error.response.status === 400) {
            console.log(`there is an error ${error.message}`)
 ...
 const getAttendants  = () => {
    ...
    axios.get("http://localhost:5000/nurse/get_nurses",{params: {page,pagesize}}).then((data:any)=> {
        ...
        <div className={styles.service_list}>
            {
                ...
                : <div className={styles.check}>
                {
                    ...
                    : <div className={styles.message}>No Nurses </div>
                }
            </div>
            }
        </div>
